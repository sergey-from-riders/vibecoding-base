...
Этот документ обязателен для всех изменений в Python React Postgres Framework.
...
Каждый backend-модуль (`apps/api/app/modules/<module>`) обязан иметь:
...
1. `verify-backend`
- lint/static checks;
- python limits check (`tools/scripts/check_python_limits.sh apps/api`);
- unit tests;
